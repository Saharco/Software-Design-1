...
class ChannelsManager(private val dbMapper: DatabaseMapper) {
    ...
    private val channelsByUsersStatistics = dbMapper.getStatistics("channels_by_users")
    private val channelsByActiveUsersStatistics = dbMapper.getStatistics("channels_by_active_users")
    private val usersByChannelsStatistics = dbMapper.getStatistics("users_by_channels")
    ...
    fun channelJoin(token: String, channel: String) {
        ...
        channelsByUsersStatistics.update(channel, usersCount, usersCount - 1, channelCreationCounter)
        channelsByActiveUsersStatistics.update(channel, onlineUsersCount, onlineUsersCount - 1, channelCreationCounter)
        usersByChannelsStatistics.update(tokenUsername, usersChannelsCount,
    ...
    fun topKChannelsByUsers(k: Int = 10): List<String> {
        return channelsByUsersStatistics.topK(k)
    }


    fun topKChannelsByActiveUsers(k: Int = 10): List<String> {
        return channelsByActiveUsersStatistics.topK(k)
    }


    fun topKUsersByChannels(k: Int = 10): List<String> {
        return usersByChannelsStatistics.topK(k)
    }
    ...
    private fun expelChannelMember(username: String, channel: String) {
        ...
        channelsByUsersStatistics.update(channel, usersCount, usersCount + 1, channelCreationCounter,
                usersCount <= 0)
        channelsByActiveUsersStatistics.update(channel, onlineUsersCount, prevOnlineUsersCount, channelCreationCounter)
        usersByChannelsStatistics.update(username, usersChannelsCount, usersChannelsCount + 1, userCreationCounter)
    }
